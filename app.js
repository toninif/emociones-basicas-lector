...
function clearReading(label='Sin rostro visible', note='Ubicate de frente, con buena luz.') {
  scores=null;lastResult=null;smoothPoints=null;ctx.clearRect(0,0,canvas.width,canvas.height);reading={label,note};renderReading();
  $('framing').hidden=true;
}
...
function draw(result) {
  ...
  if(!result || !$('points').checked)return;
...
async function loop(token){
  ...
  if(!paused){try{analysisFrame.getContext('2d').drawImage(video,0,0,canvas.width,canvas.height);const result=await faceapi.detectSingleFace(analysisFrame,new faceapi.TinyFaceDetectorOptions({inputSize:320,scoreThreshold:.5})).withFaceLandmarks().withFaceExpressions();if(token!==generation||paused)return;frozen.getContext('2d').drawImage(analysisFrame,0,0);if(result)display(result);else clearReading();}catch(error){if(token!==generation)return;stop();status('No se pudo analizar el video. Probá activar nuevamente la cámara. '+error.message,true);return;}}
